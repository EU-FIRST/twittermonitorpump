...
/*REM*/ DECLARE @numTermsTimeSlot INT = 5
...
--ADD DECLARE @numTermsTimeSlot INT = {7}
  ...
  FROM (SELECT Topic AS TopicId,
       ...
       (SELECT TOP (@numTermsTimeSlot)
         ...
         WHERE E.Name = @entity
               ...
               AND C.StartTime >= @dateTimeStart
               AND C.EndTime <= @dateTimeEnd
               AND C.Topic = TopicsOverTime.TopicId
               AND T.RecordState = 0
               AND ( 0=1
         ...
         GROUP BY T.StemHash,
                  DATEDIFF(hour, @dateTimeStart, C.StartTime)/@stepTimeSpan
        HAVING DATEDIFF(hour, @dateTimeStart, C.StartTime)/@stepTimeSpan = TopicsOverTime.TimeSlotGroup
